-- Removes cta_subheading from enforce_content_permission()'s
-- linked-counselor allowlist (added by 0054_linked_counselor_full_page_
-- access.sql, same day) — client decision made right after that pass
-- shipped: unlike cta_heading/cta_button_text, which stay ungated for
-- every role, cta_subheading is the one field the agency wants to
-- write itself, with a linked counselor limited to suggesting a change
-- (see the new SuggestEdit block on that field in page-copy.astro).
-- cta_heading/cta_button_text/plan_steps/faqs/images all stay in the
-- allowlist untouched.
